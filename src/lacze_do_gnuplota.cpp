 ...
 *              byæ wskaŒnikiem do wskaŒnika na ³añcuch: " ".
...
std::string LaczeDoGNUPlota::ZapiszUstawienieRotacjiISkali() const
{
 ...
 // Metoda usuwa i-ta nazwe pliku z listy nazw plikow
 void LaczeDoGNUPlota::UsunNazwePliku(unsigned int i) {
   unsigned int j;   
   std::list<InfoPlikuDoRysowania>::iterator Nazwy=_InfoPlikow.begin(); 
   if (i>=_InfoPlikow.size()) {
     cerr << "Blad: Niepoprawny indeks pliku na liscie plikow"<< endl;
     exit(1);
   }
   for (j=0;j<=i;j++) {
     Nazwy++;
   }
   _InfoPlikow.erase(Nazwy);
 }


 //  Metoda pozwala na odczytanie i-tej nazwy pliku wczytanej do 
 //     LaczeDoGnuplota
 const std::string LaczeDoGNUPlota::WezNazwePliku(unsigned int i) const {
   unsigned int j;   
   if (i>=_InfoPlikow.size()) {
     cerr << "Blad: Niepoprawny indeks pliku do rysowania"<< endl;
     exit(1);
   }
   std::list<InfoPlikuDoRysowania>::iterator Nazwy=_InfoPlikow.begin(); 
   for (j=0;j<=i;j++) {
...
/*  WCZYTAJ DO GNUPLOTA
    ...
    Funkcja pozwala na usuniecie wszystkich wczytanych plikow z danymi do 
    GnuPlota oraz wczytanie ich na nowo                      */
void LaczeDoGNUPlota::WczytajDoGnuPlota(const ListaPrzeszkod &Lista) {
  unsigned int i;
  const char *Plik;
  UsunWszystkieNazwyPlikow();
  DodajNazwePliku("manipulator.dat",PzG::RR_Ciagly,6);
  DodajNazwePliku("manipulator.dat",PzG::RR_Punktowy,2);
  for (i=1; i<Lista.size(); i++) {
    Plik=Lista[i]->DajNazwePliku().data();
    DodajNazwePliku(Plik,PzG::RR_Ciagly,5); 
  }
  Rysuj();   
}
